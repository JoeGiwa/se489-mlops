...
def test_predict_on_valid_input():
    ...
    input_sample = np.random.rand(1, 128, 660).astype("float32")  # Adjust shape if needed
    ...
    assert prediction.shape == (1, 10), "Expected prediction shape of (1, 10) for 10 genres"
    assert np.all(prediction >= 0) and np.all(prediction <= 1), "Probabilities must be in [0, 1]"
